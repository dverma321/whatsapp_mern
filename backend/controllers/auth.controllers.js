...
const signupUser = async (req, res) => {
    ...
    try {
        ...
        if(password != cpassword)
        {
           ...
           if(userPresent)
           {
            res.json({ message:"username is already exits"})
            return res.status("FAILED").message('User name is already Exits...')
           }

           if(emailPresent)
           {
            res.json({ message:"email is already exits"})
            return res.status("FAILED").message('Email is already Exits...')
           }

           if(phonePresent)
           {
            res.json({ message:"phone number is already exits"})
            return res.status("FAILED").message('Phone number is already Exits...')
           }
           ...
           if(userCreated)
           {
            ...
            generateJsonWebToken(userCreated._id, res);
 ...
 const loginUser = async (req, res) => {
    ...
    try {
        ...
        if(checkingHashedPassword)
        {
            ...
            generateJsonWebToken(userPresent._id, res); // passing two parameters first one is userId, secondone is res
